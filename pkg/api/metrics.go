...
func configMetricsRouter(router gin.IRouter) {
	newPrometheusRouteRegister(defaultSubsystem).ConfigRoute(router.(*gin.Engine))
}
...
func newPrometheusRouteRegister(subsystem string, customMetricsList ...[]*Metric) *Prometheus {
...
func (p *Prometheus) sendMetricsToPushGateway(metrics []byte) {
	...
	if _, err = client.Do(req); err != nil {
		glog.Errorf("Error sending to push gateway: %s\n", err.Error())
	}
}
...
func (p *Prometheus) registerMetrics(subsystem string) {
	for _, metricDef := range p.MetricsList {
		...
		if err := prometheus.Register(metric); err != nil {
			glog.Errorf("%s could not be registered in Prometheus with err: %s", metricDef.Name, err.Error())
		}
		switch metricDef {
		case reqCnt:
...
func (p *Prometheus) ConfigRoute(e *gin.Engine) {
	e.Use(p.HandlerFunc())
	p.SetMetricsPath(e)
}

// UseWithAuth adds the middleware to a gin engine with BasicAuth.
func (p *Prometheus) RegisterRouteWithAuth(e *gin.Engine, accounts gin.Accounts) {
	e.Use(p.HandlerFunc())
